...
function createBaseEvent(
  overrides: Partial<ToeicWebSyncEvent> &
    Pick<ToeicWebSyncEvent, 'event_type' | 'entity_type' | 'entity_id' | 'payload'>,
): ToeicWebSyncEvent {
  return {
...
export function appendPracticeAttempt(
  ...
  const attemptEvent = createBaseEvent({
    ...
    payload: {
      ...
      part: session.part,
      skill_tag: skillTag,
      topic: `${session.setTitle} RC practice`,
      question_count: answeredCount,
      correct_count: correctCount,
      accuracy,
      time_pressure: 'none',
      next_action:
        wrongCount > 0 ? `Review ${wrongCount} wrong items` : 'Keep review cadence',
      attempt_id: attemptId,
      origin: 'rc-practice',
  ...
  const answerEvents = answeredItems
    .map((item) => {
      ...
      return createBaseEvent({
        event_type:
          session.sourceKind === 'official' ? 'rc_item.answered' : 'drill_item.answered',
