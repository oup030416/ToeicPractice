...
export function MaterialsEditor({
  ...
  const effectiveSelection =
    selection?.group === 'official_sets' && officialSets[selection.index]
      ? selection
      : selection?.group === 'drill_sets' && drillSets[selection.index]
        ? selection
        : officialSets.length > 0
          ? { group: 'official_sets' as const, index: 0 }
          : drillSets.length > 0
            ? { group: 'drill_sets' as const, index: 0 }
            : null
  ...
  return (
    <div className="space-y-5">
      ...
      {effectiveSelection ? (
        <MaterialItemEditor
          draft={draft}
          key={`${effectiveSelection.group}-${effectiveSelection.index}-${JSON.stringify(
            effectiveSelection.group === 'official_sets'
              ? officialSets[effectiveSelection.index]
              : drillSets[effectiveSelection.index],
          )}`}
...
function MaterialItemEditor({
  ...
  draft: unknown
  selection: { group: 'official_sets' | 'drill_sets'; index: number }
